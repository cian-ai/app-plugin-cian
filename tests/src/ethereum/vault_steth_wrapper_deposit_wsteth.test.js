...
nano_models.forEach(function (model) {
  ...
  test('[Nano ' + model.letter + " " + testNetwork.name + '] Vault Steth Wrapper Deposit Wsteth', zemu(model, testNetwork.name, async (sim, eth) => {
    const contract = new ethers.Contract(contractAddr, abi);
    const stEthIn = parseUnits("20000000000000000000", 'wei');
    const to = SPECULOS_ADDRESS;
    // const to = RANDOM_ADDRESS;
    const { data } = await contract.populateTransaction.depositWstETH(stEthIn, to);
    ...
    await sim.navigateAndCompareSnapshots('.', testNetwork.name + '/' + model.name + '_vault_steth_wrapper_deposit_wsteth', [right_clicks, 0]);
    await tx;
  }));
});
