...
nano_models.forEach(function (model) {
  ...
  test('[Nano ' + model.letter + " " + testNetwork.name + '] Vault Reth Deposit', zemu(model, testNetwork.name, async (sim, eth) => {
    ...
    await sim.navigateAndCompareSnapshots('.', testNetwork.name + '/' + model.name + '_vault_reth_deposit', [right_clicks, 0]);
    await tx;
  }));
});
