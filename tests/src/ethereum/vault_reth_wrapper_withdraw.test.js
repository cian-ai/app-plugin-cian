...
nano_models.forEach(function (model) {
  ...
  test('[Nano ' + model.letter + " " + testNetwork.name + '] Vault Reth Wrapper Withdraw', zemu(model, testNetwork.name, async (sim, eth) => {
    ...
    await sim.navigateAndCompareSnapshots('.', testNetwork.name + '/' + model.name + '_vault_reth_wrapper_withdraw', [right_clicks, 0]);
    await tx;
  }));
});
