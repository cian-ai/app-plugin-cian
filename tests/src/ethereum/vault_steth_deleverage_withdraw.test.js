...
nano_models.forEach(function (model) {
  ...
  test(
    '[Nano ' + model.letter + " " + testNetwork.name + '] Vault Steth Deleverage Withdraw',
    zemu(model, testNetwork.name, async (sim, eth) => {
      const contract = new ethers.Contract(contractAddr, abi);
      const protocolId = 0
      const withdrawToken = '0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84'
      ...
      await sim.navigateAndCompareSnapshots(
        '.',
        testNetwork.name + '/' + model.name + '_vault_steth_deleverage_withdraw',
        [right_clicks, 0]);
      await tx;
    }));
});
