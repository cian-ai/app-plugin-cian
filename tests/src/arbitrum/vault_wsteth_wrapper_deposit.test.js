...
const contractAddr = "0xf2254b4da1d68540b855e4d219aac1ef0edba575";
...
nano_models.forEach(function (model) {
  ...
  test('[Nano ' + model.letter + " " + testNetwork.name + '] Vault Wsteth Wrapper Deposit', zemu(model, testNetwork.name, async (sim, eth) => {
    ...
    await sim.navigateAndCompareSnapshots('.', testNetwork.name + '/' + model.name + '_vault_wsteth_wrapper_deposit', [right_clicks, 0]);
    await tx;
  }));
});
